...
function model()
    ...
    for sim_time = dt:dt:sim_length
        ...
        for neuron = 1:num_neurons
            ...
            for channel = 1:num_channels
                ...
                if (neurons(neuron).channels(channel).I ~= abs(neurons(neuron).channels(channel).I) && neurons(neuron).channels(channel).I ~= -abs(neurons(neuron).channels(channel).I))
                    disp ('Current has complex component. Stopping.')
        ...
        for neuron = 1:num_neurons
            ...
            V_hist(neuron, 1, time_step) = neurons(neuron).compartments(1).voltage;
            V_hist(neuron, 2, time_step) = neurons(neuron).compartments(2).voltage;
            
            neurons(neuron).compartments(1).voltage
        end
    ...
    figure
    hold on
    plot(squeeze(V_hist(1,1,:)),'r')
    plot(squeeze(V_hist(2,2,:)),'g')
    plot(squeeze(V_hist(1,2,:)),'b')
    plot(squeeze(V_hist(2,2,:)),'c')
    hold off
