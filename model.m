...
function model()
    ...
    I_inj               = [0 10000*10^-9];       % [Amp] Current injection [AB_soma PD_soma]
...
%                   0  0          22.5  60  4.38  0.219  1576.8  251.85  39.42  0   ]*10^-6;   % PD    
    ...
    m_hist = zeros(num_neurons, num_channels+1,     ceil(sim_length/dt));
    h_hist = zeros(num_neurons, num_channels+1,     ceil(sim_length/dt));
    g_hist = zeros(num_neurons, num_channels+1,     ceil(sim_length/dt));
    ...
    for sim_time = dt:dt:sim_length
        ...
        if sim_time >= 389.59*10^-3 && sim_time <= 389.61*10^-3
            disp('Entering trace mode...')
            sim_time
            time_step
        end
...
%                         neurons(neuron).compartments(compartment).voltage = V + dV;
        ...
        for neuron = 1:num_neurons       
            ...
            switch integration
                ...
                case 'ExpEuler'
                    %neurons(neuron).Ca = Ca_steady_state + (Ca - Ca_steady_state)*exp(-dt/tau_Ca);  %inf + (curr - inf)*exp(-dt/tau);  % BUG: I_Ca not included in this calculation.
    ...
    hold on
        ...
        if neuron==AB, plot( (dt:dt:sim_length)*1000, squeeze(I_hist(neuron,      proc,:))*10^9, 'r.'), end % No proctolin current in PD.
    ...
    hold on
        ...
        if neuron==AB, plot( (dt:dt:sim_length)*1000, squeeze(m_hist(neuron,      proc,:)), 'r.'), end % No proctolin current in PD.
        if neuron== AB
