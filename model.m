...
%                   0  0          22.5  60  4.38  0.219  1576.8  251.85  39.42  0   ]*10^-6;   % PD    
    ...
    for sim_time = dt:dt:sim_length
        ...
        for neuron = 1:num_neurons
            ...
            for channel = 1:num_channels
                if channel <= 2, compartment = Soma; else compartment = AIS; end % First two channels are in axon initial segment.
        ...
        for neuron = 1:num_neurons
            ...
            if axial_current ~= 0, STOP; end %%%%% TESTING uncoupled compartments should have no axial current.
...
%                V = neurons(neuron).compartments(compartment).voltage;
                ...
                disp('+++++++++Starting new channel')
                if channel<4 %%%%%%%%%% TESTING
...
%                     end
                ...
                disp ('++++++++Added soma leak conductance.')
                neuron
                compartment
                conductance_sum                
        ...
        for neuron = 1:num_neurons       
            I_CaT =  0; %%%%% TESTING neurons(neuron).channels(3).I;
            I_CaS =  0; %%%%% TESTING neurons(neuron).channels(4).I;            
            I_Ca  = I_CaT + I_CaS; 
            Ca = neurons(neuron).Ca;
